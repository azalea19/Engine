	...
	for i = 1, numRows do
		if(fileType == "gameObject") then
			if data[i]["currentHealth"] == nil then
				...
				saveTable[#saveTable + 1] =  ","
				saveTable[#saveTable + 1] =  data[i]["displayNameOnLook"]
				if(data[i]["animation"][1] ~= nil)
					saveTable[#saveTable + 1] =  data[i]["animation"][1]
					saveTable[#saveTable + 1] =  ","
				else
					saveTable[#saveTable + 1] =  "none"
					saveTable[#saveTable + 1] =  ","	
				end
				if(data[i]["animation"][2] ~= nil)
					saveTable[#saveTable + 1] =  data[i]["animation"][2]
					saveTable[#saveTable + 1] =  ","
				else
					saveTable[#saveTable + 1] =  "none"
					saveTable[#saveTable + 1] =  ","	
				...
				saveTable[#saveTable + 1] =  "\n"
				data[i]["displayNameOnLook"]
				data[i]["animation"][1,2,3]
				data[i]["objType"]
				data[i]["setBaseTransform"][position/direction/scale][x/y/z]
			end
		else
			if(fileType == "npc") then
				if data[i]["currentHealth"] ~= nil then
					...
					saveTable[#saveTable + 1] =  data[i]["hurtAnim"]
					saveTable[#saveTable + 1] =  ","

					data[i]["hurtAnim"]
					data[i]["defaultAnim"][1,2,3] some may be null
					data[i]["currentStateID"]
					data[i]["hostileToPlayer"]
					--saveTable[#saveTable + 1] =  ","
